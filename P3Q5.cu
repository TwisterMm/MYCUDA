...
int main(){
    ...
    cudaMemcpy(d_m3, h_m3, MAX_BYTES, cudaMemcpyHostToDevice);

    //calling kernel
    matrix_mul <<< CEIL(MAX_SIZE, 1024), 1024 >>> (d_m1, d_m2, d_m3);

    //transferring result from device to host
    cudaMemcpy(h_m1, d_m1, MAX_BYTES, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_m2, d_m2, MAX_BYTES, cudaMemcpyDeviceToHost);
